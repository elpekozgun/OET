     ...
     +  vdesired,lambda,rhs,fold,rhsold,frate,dtfactor
     ...
     +  nodeelement(nnmax,100),readacc,nbase,base(10,2)
     
      integer nn,nel,steelflag,readdamage,i,j,m,n,ikt,k,iii,
     +  i1,i2,i3,i4,i5,externalloadtype,nloads,loading(10,10,4),
...
c	EA=ff/(0.5*energyn(1)+0.5*energyn(2))
	EA=ff/(energyn(1))
      write(*,*) 'EA ',EA
...
c
      ...
      do i=1,maxntimedata
         read(13,*,end=244) i1,i2,i3,i4,i5
         do j=i1,i2,i3
...
241   continue
     ...
     +                beta2*rhs
              ...
              f=0
c      write(71,*) tt,f
c      write(72,*) tt,rr(nvcont)
c      write(*,*) f,fold,nvcont,rr(nvcont)
c     
c      if(dtfactor.lt.-0.125e-03)then
c        dtfactor=-0.125e-03
c        tfactorr=dtfactor/dt
c      endif
c      if(dtfactor.gt.0.125e-03)then
c        dtfactor=0.125e-03
c        tfactorr=dtfactor/dt
c      endif
            ...
            if(loadingtype(i).eq.2)then
              do j=1,ntri(i)
                 ...
                 if(i1.ne.0)then
                   ...
                   sum=sum+triff(i,j)*ff*trif(i,3)
c              write(74,*) xcoord(nntri(i,j),2),triff(i,j)*ff*trif(i,3)
                 endif
                 i1=nf(nntri(i,j),2)
                 if(i1.ne.0)then
...
c
      ...
      do i=1,n
        u(i)=u(i)+dt*v(i)+beta1*loadsold(i)+beta2*loads(i)
        v(i)=v(i)+gamma1*loadsold(i)+gamma2*loads(i)
        loadsold(i)=loads(i)       
      enddo
      ...
      maxu=0.
      do i=1,nn
        if(nf(i,1).ne.0.and.nf(i,2).ne.0)then
            uu=dsqrt(u(nf(i,1))**2.+u(nf(i,2))**2.)
            vv=dsqrt(v(nf(i,1))**2.+v(nf(i,2))**2.)
            vvv(i)=vv
            uuu(i)=uu
            dispx(i)=u(nf(i,1))
            dispy(i)=u(nf(i,2))
      ...
      if(mod(ikt,pfreq2).eq.0)then
        ...
        write(*,876) ikt,tt,maxu,maxv,maxa,averagea,nfailed
      endif
876   format(i8,5(1pe12.3),i6)   
